...
가벼운 뉴스 요약 도구가 아니다. 회사 평판·경쟁사 동향은 **하나라도 놓치면 안 되는** 영역이라,
매일 외신·국내 보도를 구글뉴스까지 훑어 **전수 수집**하고 — 분류·중요도 판정·교차 합성을 거쳐 —
정기 브리핑으로 만든다(이메일 사내 발송은 옵션). 두 가지 산출물:

| 산출물 | 무엇을 답하나 | 내용 |
|--------|--------------|------|
| **자사 보도 모니터링** (PR) | "우리가 어떻게 보도됐나" | 자사 언급 기사 + 톤(긍/중/부) + **언급유형**(직접/간접)·맥락 1줄 + 통계 + 월별 엑셀 |
| **업계 인사이트 뉴스레터** | "업계에 무슨 일이 — 우리는 뭘 해야 하나" | 외부 동향 + **자사 전략적 대응점(시사점)** |

**설정은 `/setup` 한 번.** 자동 리서치가 자사 맥락·경쟁사·키워드·소스를 조사해 워크스페이스에
깔아주고(필요하면 직접 보강), 이메일 발송할 거면 관리자 키만 넣으면 끝. 이후엔 **돌릴 때마다
자사·경쟁사 맥락이 `data/self-context/` 에 쌓여**, 다음 브리핑의 전략적 시사점이 그 누적 맥락을
참고해 점점 쓸모 있어진다 — 단발 요약과 결정적으로 다른 지점이다.

> 이 "전수 수집 → 분류 → 합성"이 무거운 이유: 일반 뉴스 가져오기가 아니라 **누락 없는 모니터링**이
> 목표이기 때문이다. 그래서 LLM과 다단 전처리를 거친다.
...
`/newsletter` 한 번이면 아래 형식의 HTML 뉴스레터가 만들어지고(이메일 인증 시 발송까지) 끝난다.
*(구조 예시 — 실제 회사·카테고리명은 도메인팩에서 결정)*

```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 [회사] · INDUSTRY INSIGHT                 2026-06-15
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 TL;DR  이번 기간을 관통하는 한 문장. 그래서 자사엔 무엇이 달라지는지 한 문장.

 인사이트
  ① [거시적 통찰 한 문장 — "그래서 우리는…"이 나오는 제목]
     관찰     외부 기사 2~4건을 한 줄기로 교차 [1][2][3]
     자사 함의  경쟁 기준선 대비 자사 위치 + 구체 함의
  ② …    ③ …
 ...
 이번 호 등장 기업   생소한 회사에 1줄 설명  ·   출처 (N건)  모든 주장 URL 검증 가능
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```

`/pr-clipping` 은 같은 방식으로 **자사 언급 기사 + 톤 라벨(긍정/중립/부정) HTML + 월별 엑셀**을 만든다.
...
<sub>파란색 = 결정론 코드(LLM 없음) · 주황색 = LLM. 수집·추출·분류·검증·렌더는 코드가, 중요도 채점·인사이트 합성만 LLM이 한다.</sub>
...
| `/pr-clipping [date] [hours]` | 자사 PR 클리핑 생성·발송 (`hours`=수집창 시간; 생략 시 정책값) |
