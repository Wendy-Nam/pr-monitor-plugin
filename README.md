...
설치 시 이메일 인증값을 물어봅니다 — 입력하면 키체인에 안전하게 저장됩니다(건너뛰어도 되며, 그 경우 이메일 발송만 비활성화됩니다).
이메일 채널은 둘 중 하나입니다(`delivery.yaml` 의 `email.provider`):
- **microsoft_graph** (Azure AD): `azure_tenant_id`/`azure_client_id`/`azure_client_secret`/`email_from`
- **smtp** (Gmail·O365·AWS SES·사내 메일 등 표준 SMTP): `email.smtp` 의 host·port·user·password — Azure 없이도 발송 가능
