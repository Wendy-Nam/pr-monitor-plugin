...
등록하면 자동으로 실행됩니다. **스케줄 정보는 패키지에 포함되지 않으므로 설치 시 한 번 등록해야 합니다.**
루틴은 데스크탑 앱이 켜져 있을 때만 발화합니다.

## 한계 (솔직하게)

- **품질의 천장은 사람 손에 달려 있습니다**: 인사이트 합성에 쓰는 예시(`config/prompt-examples.yaml`)는
  자동 생성되지 않습니다. 새 회사는 형식은 강제되지만, 글의 품질은 운영하며 직접 다듬어야 합니다.
- **Cowork(클라우드) 미지원**: 클라우드 샌드박스가 외부 수집을 차단합니다. 로컬 데스크탑 전용입니다.
